...
class RejectedUsersActivity : AppCompatActivity() {
    ...
    private val allUsers = ArrayList<Pair<Long, RegisteredUser>>()
    private val groups = ArrayList<RegisteredUserGroup>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = RegisteredUsersAdapter(
            groups,
            onUserClick = { user ->
                val intent = Intent(this, RejectedUserDetailsActivity::class.java)
    ...
    private fun passesFilter(label: String): Boolean {
        return when (currentFilter) {
            "TODAY" -> label == "TODAY"
            "WEEK" -> label == "TODAY" || label == "YESTERDAY" || label == "THIS WEEK"
            "OLDER" -> label != "TODAY" && label != "YESTERDAY" && label != "THIS WEEK"
            else -> true
        }
    }
        ...
        database.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (user in snapshot.children) {
                    val status = user.child("kycStatus").getValue(String::class.java)
                    if (status != "rejected") continue

                    val phone = user.key ?: ""
                    val name = user.child("name").getValue(String::class.java) ?: "Unknown"
                    val created = user.child("created_time").getValue(Long::class.java) ?: 0L
                    ...
                    allUsers.add(Pair(created, registeredUser))
                }
                allUsers.sortByDescending { it.first }
                txtUserCount.text = allUsers.size.toString()
                filterUsers("")
            }
    ...
    private fun filterUsers(query: String) {
        val groupedMap = LinkedHashMap<String, MutableList<RegisteredUser>>()
        val groupedTime = LinkedHashMap<String, Long>()

        for ((time, user) in allUsers) {
            val matchesQuery = user.name.contains(query, true) || user.phone.contains(query, true)
        ...
        val newGroups = ArrayList<RegisteredUserGroup>()
    ...
    private fun getGroupLabel(timestamp: Long): String {
        ...
        if (calendar.get(java.util.Calendar.YEAR) == userCalendar.get(java.util.Calendar.YEAR) &&
        ...
        if (calendar.get(java.util.Calendar.MONTH) == userCalendar.get(java.util.Calendar.MONTH) &&
            calendar.get(java.util.Calendar.YEAR) == userCalendar.get(java.util.Calendar.YEAR)
        ) {
    ...
    private fun getSubtitle(label: String, timestamp: Long): String {
        return when (label) {
            "TODAY", "YESTERDAY" -> java.text.SimpleDateFormat("dd MMM yyyy", java.util.Locale.getDefault()).format(java.util.Date(timestamp))
            "THIS WEEK" -> "Users registered in the last 7 days"
            "EARLIER THIS MONTH" -> "Users registered this month"
            else -> "Archived users"
        }
    }
}